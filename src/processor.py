...
class processor(object):
    ...
    def set_optimizer(self):
        ...
        self.optimizer = NoamOpt(
            32,
            self.args.factor,
            self.dataloader.trainbatchnums * self.args.warmup,
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            if epoch >= self.args.start_test:
                ...
                test_error, test_final_error = self.test_epoch()
                self.best_ade = (
                    test_error if test_final_error < self.best_fde else self.best_ade
                )
                self.best_epoch = (
                    epoch if test_final_error < self.best_fde else self.best_epoch
                )
                self.best_fde = (
                    test_final_error
    ...
    def test_epoch(self):
        ...
        for batch in tqdm(range(self.dataloader.testbatchnums)):
            ...
            lossmask, num = getLossMask(
                all_output, seq_list[0], seq_list[1:], using_cuda=self.args.using_cuda
            )
            error, error_cnt, final_error, final_error_cnt = L2forTestS(
                all_output, batch_norm[1:, :, :2], self.args.obs_length, lossmask
            )
